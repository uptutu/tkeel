...
func TestDefaultOpratorHttpMethod(t *testing.T) {
	t.Run("test default oprator http method", func(t *testing.T) {
		...
		assert.NoError(t, err)
		iresp := &IdentifyResp{}
		assert.NoError(t, readutil.ReaderToJSON(resp.Body, iresp))
		assert.NotNil(t, iresp)
		assert.Equal(t, iresp.Ret, 0)
		assert.Equal(t, iresp.Msg, "ok")
		assert.Equal(t, iresp.PluginID, "keel-hello")
		...
		assert.NoError(t, err)
		sresp := &StatusResp{}
		assert.NoError(t, readutil.ReaderToJSON(resp.Body, sresp))
		...
		assert.NoError(t, readutil.ReaderToJSON(resp.Body, tresp))
		...
		assert.NoError(t, readutil.ReaderToJSON(resp.Body, aresp))
